...
const News = (props) => {
  ...
  useEffect(() => {
    document.title = `Mukhya Samachar - ${capitalizeFirstLetter(
      props.category
    )}`;
    updateNews();
  }, []);
  ...
  const fetchMoreData = async () => {
    let url = `https://newsapi.org/v2/top-headlines?country=${
      props.country
    }&category=${props.category}&apiKey=4f08ab3e48e549cc81fdcd93d5890d2b&page=${
      page + 1
  ...
  return (
    <>
      ...
      <h2 className="text-center" style={{ margin: "120px 0 70px 0" }}>
        Mukhya Samachar - {capitalizeFirstLetter(props.category)}
      </h2>
      {loading && <Spinner />}
